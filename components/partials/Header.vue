    ...
    >
      <template v-if="action === ''">
        <nui-container class="flex items-center lg:py-6">
          <!-- Left Title -->
          <nuxt-link to="/" class="text-3xl">NUXTUI</nuxt-link>
          <!-- <a
          ...
          <ul class="hidden lg:flex lg:pt-1 xl:pt-0 text-center mx-auto">
            ...
            >
              <nuxt-link
                class="block p-2 font-medium uppercase hover:no-underline hover:text-nuxt-lightgreen"
                :to="{ name: 'section-slug', params: { section: link } }"
              >
                {{ $store.state.lang.links[link] || link }}
              </nuxt-link>
            </li>
            <li
              class="header_nav_link xl:px-4 lg:py-0 lg:px-2 py-2 pd"
            >
              <div
                class="block p-2 font-medium uppercase hover:no-underline hover:text-nuxt-lightgreen"
              >
          ...
          <a
            href="#"
            class="block md:hidden flex p-2 -m-2 mr-4 items-center justify-center text-nuxt-gray hover:text-nuxt-lightgreen z-10 lg:hidden"
      ...
      <nui-container
        v-else
        class="flex justify-between lg:hidden"
      >
        <!-- Left Title -->
        <nui-search
          v-if="action === 'search'"
        ...
        >
          <component
            :is="'nui-' + action + '-icon'"
            class="block h-6 text-nuxt-lightgreen fill-current"
          />
        ...
        <a
          href="#"
          class="block flex p-2 -m-2 items-center justify-center text-nuxt-gray hover:text-nuxt-lightgreen z-10 lg:hidden"
    ...
    <nav class="header_mobile_nav block lg:hidden">
      <div class="flex justify-between">
        <nuxt-link
          ...
          :to="{ name: 'section-slug', params: { section: link } }"
          @click.prevent.native="$emit('change', action === link ? '' : ($route.params.section !== link ? '' : link))"
        >
          <component
            :is="'nui-' + link + '-icon'"
            class="inline-block h-5 fill-current mb-1"
            :class="{'text-nuxt-lightgreen': action === link}"
          />
      ...
      <nui-container>
        ...
        >
          ...
          >
            ...
            >
              <nuxt-link
                class="block text-gray-700 hover:text-nuxt-lightgreen"
